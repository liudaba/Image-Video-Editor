...
try:
    from video_generator.multi_model import warmup_model
except ImportError:
    try:
        from video_generator.ollama_client import warmup_model
    except ImportError:
        warmup_model = None
...
class ShotsMixin:
    def _safe_release_whisper_gpu(self):
        ...
        try:
            if self.whisper_model is not None and getattr(self, '_whisper_on_gpu', False):
                import torch
    ...
    def preload_whisper_model(self):
        """预加载Whisper模型到内存（启动时调用，加载到CPU避免占用GPU显存）"""
        try:
            import whisper
            import torch
...
返回："""
    ...
    def generate_shots(self, auto_mode=False):
        ...
        try:
            ...
            # 修复：重置增强识别器的上下文，避免上一次任务的残留影响
            if ENHANCED_RECOGNITION_AVAILABLE:
                try:
                    get_enhanced_recognizer().reset_context()
                except Exception:
                    pass
