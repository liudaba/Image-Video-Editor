...
NEW_VERSION = "1.0.11"
FROM_VERSIONS = ["1.0.0", "1.0.1", "1.0.2", "1.0.3", "1.0.4", "1.0.5", "1.0.6", "1.0.7", "1.0.8", "1.0.9", "1.0.10"]

CHANGELOG = [
    "修复任务取消失效问题：按停止按钮后程序不再继续运行",
    "LLM调用添加取消检查点，支持在请求前和模型循环中中断",
    "6个call_ollama_single调用点全部添加cancel_check参数",
    "优化提示词模板性别中性规则，修复生成图片出现女人头像问题",
    "优化Flux/SD3模型适配，自然语言模型保留自然描述",
    "统一非写实风格检测关键词(NON_REALISTIC_KEYWORDS)",
]

CHANGED_FILES = [
    "video_generator/ollama_client.py",
...
def create_patch_zip(from_ver):
    ...
    print(f"  补丁: {os.path.basename(zip_path)} ({size/1024:.1f} KB, sha256={sha256[:16]}...)")
    ...
    print(f"  manifest.json files格式验证: {type(manifest_obj['files'])} (应为list)")
    for item in manifest_obj["files"]:
        assert "path" in item and "sha256" in item and "size" in item, f"manifest条目缺少字段: {item}"
    print(f"  manifest.json 条目验证通过 ({len(manifest_obj['files'])}个文件)")
...
print("\n[5/5] 验证：模拟客户端检查更新")
for from_ver in ["1.0.10", "1.0.9", "1.0.5", "1.0.0"]:
