...
def get_whisper_model_path(model_name):
    """获取Whisper模型路径，优先从打包目录查找，回退到用户缓存目录
    
    打包版: {app_dir}/whisper_models/{model_name}.pt
    源码版: ~/.cache/whisper/{model_name}.pt
    """
    import os
    app_dir = get_app_dir()
    local_model = os.path.join(app_dir, "whisper_models", f"{model_name}.pt")
...
def get_ffmpeg_dir():
    """获取FFmpeg目录，优先从打包目录查找，回退到系统PATH
