...
4. 按格式输出结果"""
    ...
    _COMMON_RULES = (
        ...
        "- 'tian jiang shen bing' (divine troops descending) = unexpected rescue/savior -> show 'no help arriving, standing alone', NOT 'ancient cannon' or 'soldiers falling from sky'\n"
        "- 'kun zai zhan che shang' (bound to a chariot) = interests tied together -> show 'generals surrounded by wealth tied to leader', NOT 'ropes and restraints on people'\n"
        "- 'ya cang shi' (ballast stone) = stabilizing force/foundation -> show 'military as unshakable foundation of power', NOT 'a person holding a stone'\n"
        "- 'zou gang si' (walking tightrope) = precarious balance -> show 'diplomat carefully navigating between opposing forces', NOT 'hand balancing globe on rope'\n"
        "- 'man xing bing' (chronic disease) = slowly worsening problem -> show 'gradually deteriorating situation, cracks widening over time', NOT 'medical examination or stethoscope'\n"
        "- 'tie tong' (iron barrel) = airtight protection -> show 'impenetrable wall of guards around power center', NOT 'a metal barrel'\n"
        "- 'hou lu bei du si' (retreat blocked) = no way out -> show 'all exits sealed, trapped with no escape route', NOT 'broken telegraph machine'\n"
        "- 'ya zhu' (chips on the table) = leverage/bargaining power -> show 'using resources as leverage in negotiation', NOT 'a bronze key in stone chamber'\n"
        "- RULE: When you encounter a Chinese metaphor, ask yourself: 'What REAL SCENE does this metaphor describe in the political/social context?' Then depict THAT scene.\n"
        "\n"
