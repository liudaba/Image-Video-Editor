...
class LoginDialog(tk.Toplevel):
    ...
    def _setup_styles(self):
        ...
        style.configure("Login.TLabel", background=self._BG, foreground=self._TEXT_FG, font=("Microsoft YaHei", 11))
        style.configure("Login.Title.TLabel", background=self._BG, foreground=self._ACCENT, font=("Microsoft YaHei", 22, "bold"))
        style.configure("Login.Sub.TLabel", background=self._BG, foreground=self._HINT_FG, font=("Microsoft YaHei", 10))
        style.configure("Login.Hint.TLabel", background=self._BG, foreground=self._WARN_FG, font=("Microsoft YaHei", 10))
        style.configure("Login.Trial.TLabel", background=self._BG, foreground=self._WARN_FG, font=("Microsoft YaHei", 11, "bold"))
        style.configure("Login.Error.TLabel", background=self._BG, foreground=self._ERROR_FG, font=("Microsoft YaHei", 9))
        style.configure("Login.TCheckbutton", background=self._BG, foreground=self._TEXT_FG, font=("Microsoft YaHei", 10))
        style.configure("Login.TButton", font=("Microsoft YaHei", 11), padding=(10, 8))
        style.configure("Login.Primary.TButton", background=self._ACCENT, foreground="#ffffff", font=("Microsoft YaHei", 12, "bold"), padding=(15, 10))
        style.map("Login.Primary.TButton", background=[('active', self._ACCENT_HOVER), ('pressed', '#1565c0')])
        style.configure("Login.Link.TButton", background=self._BG, foreground=self._ACCENT, font=("Microsoft YaHei", 10), padding=(5, 4))
        style.map("Login.Link.TButton", foreground=[('active', '#64b5f6')])
        style.configure("Login.Purchase.TButton", background=self._BTN_PURCHASE, foreground="#ffffff", font=("Microsoft YaHei", 11, "bold"), padding=(10, 8))
        style.map("Login.Purchase.TButton", background=[('active', '#f57c00')])
        style.configure("Login.Secondary.TButton", background=self._BTN_SECONDARY, foreground="#ffffff", font=("Microsoft YaHei", 10), padding=(8, 6))
        style.map("Login.Secondary.TButton", background=[('active', '#505050')])

    def _make_entry(self, parent, variable, show=None, placeholder=""):
        entry_frame = tk.Frame(parent, bg=self._INPUT_BORDER, bd=1, highlightthickness=0)
        entry = tk.Entry(
            entry_frame,
        ...
        entry.pack(fill=tk.BOTH, expand=True, padx=2, pady=2, ipady=6)
        entry._frame = entry_frame
        entry.bind("<FocusIn>", lambda e: entry_frame.configure(bg=self._INPUT_FOCUS))
        entry.bind("<FocusOut>", lambda e: entry_frame.configure(bg=self._INPUT_BORDER))
        if placeholder:
    ...
    def _build_ui(self):
        main = ttk.Frame(self, style="Login.TFrame", padding=(40, 30, 40, 20))
        ...
        ttk.Label(card, text="用户名", style="Login.TLabel", background=self._PANEL_BG).pack(anchor=tk.W, pady=(0, 4))
        ...
        ttk.Label(card, text="密码", style="Login.TLabel", background=self._PANEL_BG).pack(anchor=tk.W, pady=(0, 4))
        ...
        self._agree_check = ttk.Checkbutton(
            ...
            variable=self._agree_var, style="Login.TCheckbutton",
        )
        self._agree_check.configure(background=self._PANEL_BG)
        ...
        self.action_btn.pack(fill=tk.X, pady=(4, 6))
        ...
        btn_row = ttk.Frame(bottom_frame, style="Login.TFrame")
        btn_row.pack(fill=tk.X)

        self.purchase_btn = ttk.Button(btn_row, text="💎 购买会员", command=self._show_purchase_dialog, style="Login.Purchase.TButton")
        self.purchase_btn.pack(side=tk.LEFT, padx=(0, 8))

        self.reset_btn = ttk.Button(btn_row, text="🔑 密码邮箱找回", command=self._show_reset_dialog, style="Login.Secondary.TButton")
        self.reset_btn.pack(side=tk.LEFT)

        trial_lbl = ttk.Label(main, text="✨ 注册登录7天免费试用!", style="Login.Trial.TLabel")
        trial_lbl.pack(pady=(12, 0))
    ...
    def _toggle_mode(self):
        ...
        if self.mode == "register":
            self._email_label.pack(after=self.username_entry, anchor=tk.W, pady=(0, 4))
            self.email_entry.pack(after=self._email_label, fill=tk.X, pady=(0, 12))
            self._confirm_label.pack(after=self.password_entry, anchor=tk.W, pady=(0, 4))
...
class PasswordResetDialog(tk.Toplevel):
    ...
    def _setup_styles(self):
        ...
        style.configure("Reset.TLabel", background=self._BG, foreground=self._TEXT_FG, font=("Microsoft YaHei", 11))
        style.configure("Reset.Title.TLabel", background=self._BG, foreground=self._ACCENT, font=("Microsoft YaHei", 16, "bold"))
        style.configure("Reset.Hint.TLabel", background=self._BG, foreground=self._HINT_FG, font=("Microsoft YaHei", 9))
        style.configure("Reset.TButton", font=("Microsoft YaHei", 11), padding=(10, 8))
        style.configure("Reset.Primary.TButton", background=self._ACCENT, foreground="#ffffff", font=("Microsoft YaHei", 12, "bold"), padding=(15, 10))
        style.configure("Reset.TCheckbutton", background=self._BG, foreground=self._TEXT_FG, font=("Microsoft YaHei", 10))

    def _make_entry(self, parent, variable, show=None):
        entry_frame = tk.Frame(parent, bg=self._INPUT_BORDER, bd=1)
        entry = tk.Entry(
            entry_frame, textvariable=variable, font=("Microsoft YaHei", 12),
            bg=self._INPUT_BG, fg=self._INPUT_FG, insertbackground=self._INPUT_FG,
            relief=tk.FLAT, bd=0, show=show if show else "", highlightthickness=0,
        )
        entry.pack(fill=tk.BOTH, expand=True, padx=2, pady=2, ipady=6)
        entry._frame = entry_frame
        entry.bind("<FocusIn>", lambda e: entry_frame.configure(bg=self._ACCENT))
        entry.bind("<FocusOut>", lambda e: entry_frame.configure(bg=self._INPUT_BORDER))
        return entry

    def _build_ui(self):
        main = ttk.Frame(self, style="Reset.TFrame", padding=(35, 25, 35, 20))
        main.pack(fill=tk.BOTH, expand=True)

        ttk.Label(main, text="🔑 密码找回", style="Reset.Title.TLabel").pack(pady=(0, 5))
        ttk.Label(main, text="通过注册邮箱验证身份后重置密码", style="Reset.Hint.TLabel").pack(pady=(0, 18))

        self.email_var = tk.StringVar()
        ttk.Label(main, text="注册邮箱", style="Reset.TLabel").pack(anchor=tk.W, pady=(0, 4))
        ...
        self.code_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(8, 8))
        self.send_btn = ttk.Button(btn_row, text="发送验证码", command=self._send_code, style="Reset.TButton")
        self.send_btn.pack(side=tk.RIGHT)

        self.new_pass_var = tk.StringVar()
        ttk.Label(main, text="新密码", style="Reset.TLabel").pack(anchor=tk.W, pady=(0, 4))
        self.new_pass_entry = self._make_entry(main, self.new_pass_var, show="●")
        self.new_pass_entry.pack(fill=tk.X, pady=(0, 12))

        self.confirm_pass_var = tk.StringVar()
        ttk.Label(main, text="确认新密码", style="Reset.TLabel").pack(anchor=tk.W, pady=(0, 4))
        self.confirm_pass_entry = self._make_entry(main, self.confirm_pass_var, show="●")
        self.confirm_pass_entry.pack(fill=tk.X, pady=(0, 18))

        ttk.Button(main, text="重置密码", command=self._do_reset, style="Reset.Primary.TButton").pack(fill=tk.X, pady=(0, 8))
...
class PurchaseDialog(tk.Toplevel):
    ...
    PLANS = [
        {"key": "monthly", "name": "月卡", "price": "¥19.9/月", "desc": "30天专业版"},
        {"key": "quarterly", "name": "季卡", "price": "¥49.9/季", "desc": "90天专业版"},
        {"key": "yearly", "name": "年卡", "price": "¥199/年", "desc": "365天专业版"},
        {"key": "lifetime", "name": "终身会员", "price": "¥599", "desc": "永久专业版"},
    ]
    ...
    def _setup_styles(self):
        ...
        style.configure("Purchase.TLabel", background=self._BG, foreground=self._TEXT_FG, font=("Microsoft YaHei", 11))
        style.configure("Purchase.Title.TLabel", background=self._BG, foreground=self._GOLD, font=("Microsoft YaHei", 18, "bold"))
        style.configure("Purchase.TButton", font=("Microsoft YaHei", 11), padding=(10, 8))
        style.configure("Purchase.Primary.TButton", background=self._GOLD, foreground="#1e1e1e", font=("Microsoft YaHei", 12, "bold"), padding=(15, 10))
        style.configure("Purchase.TCheckbutton", background=self._BG, foreground=self._TEXT_FG, font=("Microsoft YaHei", 10))
    ...
    def _build_ui(self):
        main = ttk.Frame(self, style="Purchase.TFrame", padding=(30, 20, 30, 15))
        main.pack(fill=tk.BOTH, expand=True)

        ttk.Label(main, text="💎 购买会员", style="Purchase.Title.TLabel").pack(pady=(0, 15))

        plans_frame = ttk.Frame(main, style="Purchase.TFrame")
        plans_frame.pack(fill=tk.X, pady=(0, 12))
        ...
        for i, plan in enumerate(self.PLANS):
            ...
            name_lbl = tk.Label(card, text=plan["name"], font=("Microsoft YaHei", 11, "bold"),
                                bg=self._PANEL_BG, fg=self._TEXT_FG)
            name_lbl.pack()
            price_lbl = tk.Label(card, text=plan["price"], font=("Microsoft YaHei", 13, "bold"),
                                 bg=self._PANEL_BG, fg=self._GOLD)
            price_lbl.pack(pady=(2, 0))
            desc_lbl = tk.Label(card, text=plan["desc"], font=("Microsoft YaHei", 9),
        ...
        ttk.Label(main, text="激活码激活", style="Purchase.TLabel").pack(anchor=tk.W, pady=(0, 4))
        activate_frame = ttk.Frame(main, style="Purchase.TFrame")
        activate_frame.pack(fill=tk.X, pady=(0, 12))

        self.activate_var = tk.StringVar()
        self.activate_entry = self._make_entry(activate_frame, self.activate_var)
        self.activate_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 8))
        ...
        sep2.pack(fill=tk.X, pady=(4, 12))

        pay_label_frame = ttk.Frame(main, style="Purchase.TFrame")
        pay_label_frame.pack(fill=tk.X, pady=(0, 4))
        ...
        pay_btn_frame.pack(fill=tk.X, pady=(0, 8))
        ttk.Button(pay_btn_frame, text="支付宝支付", command=lambda: self._do_purchase("alipay"),
                    style="Purchase.TButton").pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 4))
        ttk.Button(pay_btn_frame, text="微信支付", command=lambda: self._do_purchase("wechat"),
                    style="Purchase.TButton").pack(side=tk.RIGHT, fill=tk.X, expand=True, padx=(4, 0))

        ttk.Button(main, text="关闭", command=self.destroy, style="Purchase.TButton").pack(fill=tk.X, pady=(8, 0))
